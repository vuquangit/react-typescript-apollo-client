...
const API_ENDPOINT = 'https://push-notification-nodejs.herokuapp.com'
...
export default function usePushNotifications() {
  ...
  const onClickSendSubscriptionToPushServer = () => {
    ...
    setError(false)
    axios
      .post(`${API_ENDPOINT}/subscription`, { data: userSubscription })
  ...
  const onClickSendNotification = async () => {
    ...
    setError(false)
    axios
      .get(`${API_ENDPOINT}/subscription/${pushServerSubscriptionId}`)
      .catch((error) => {
        setLoading(false)
        setError(error)
      })
